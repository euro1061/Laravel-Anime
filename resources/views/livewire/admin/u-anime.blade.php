<div class="flex-auto bg-gray-900">
    <div class="py-6 px-3 bg-indigo-700 text-2xl text-white text-center shadow-2xl">
        เรื่อง : {{$Anime->AnimeName}}
    </div>
    <div class="mx-4 mt-5 bg-white p-5 rounded shadow-xl">
        <form wire:submit.prevent="update({{$Anime->id}})">
                ...
                <div>
                    ...
                    <div class='flex items-center justify-center w-full'>
                        <label
                            ...
                            <input type='file' class="hidden" wire:change="$emit('fileChoosenCover')" id="CoverImage" />
                        </label>
                    </div>
                </div>
                <div class="mt-5">
                    ...
                    @else
                    <img src="{{url('storage/'.$CoverImageOld)}}" alt="" class="w-full rounded-md">
                    @endif
                </div>
                    ...
                    <div class='flex items-center justify-center w-full'>
                        <label
                            ...
                            <input type='file' class="hidden" id="PosterImage"
                                wire:change="$emit('fileChoosenPoster')" />
                        </label>
                    </div>
                </div>
                <div class="mt-5">
                    ...
                    @else
                    <img src="{{url('storage/'.$PosterImageOld)}}" alt="" class="w-full rounded-md">
                    @endif
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 mt-5 p-3 bg-indigo-500 rounded-md">
                <div class="flex justify-between">
                    ...
                    <button type="button" class="py-2 px-2 bg-yellow-600 hover:bg-yellow-700 rounded text-white" wire:click.prevent="addEpisode">+ ADD</button>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-2 mt-3">
                    ...
                    <div class="grid @if ($index != 0) grid-cols-7 @else grid-cols-1 @endif">
                        ...
                        @if ($index != 0)
                            <button wire:click="deleteEpisode({{$index}} , {{$Episode['id']}})" type="button" class="bg-red-600 text-white rounded">X</button>
                        @endif
                    </div>
                </div>
                @endforeach
            ...
            <div class="mt-5" x-data="{}">
                <button @click="getTags()" type="submit"
                    class="px-2 py-3 bg-blue-800 hover:bg-blue-900 rounded-md w-full text-white">
                <span wire:loading.remove wire:target="update">SAVE</span> 
                <div class="fa-1x" wire:loading wire:target="update">
...
function dropdown() {
    return {
        options: [],
        selected: [ 
            @foreach ($AnimeTags as $TagSelect)
                {{$TagSelect-1}},
            @endforeach
        ],
        show: false,
        ...
        loadOptions() {
            ...
            for (let i = 0; i < options.length; i++) {
                ...
                /* console.log(options[i].getAttribute('selected') != null ? options[i].value : false); */
